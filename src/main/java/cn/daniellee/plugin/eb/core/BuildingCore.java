...
public class BuildingCore {
	...
	public static void teleportToBuilding(Player player, Building building) {
		if (BuildingCore.serverName.equals(building.getServer())) {
			...
			} else player.sendMessage((ExcellentBuilding.getInstance().getPrefix() + ExcellentBuilding.getInstance().getConfig().getString("message.visit-failed", "&eTemporarily unable to transfer you to the target building.")).replace("&", "§"));
		} else {
			if (ExcellentBuilding.getInstance().isBungeecord()) {
				if (BuildingCore.serverList.contains(building.getServer())) {
					...
					out.writeUTF(building.getServer());
					player.sendPluginMessage(ExcellentBuilding.getInstance(), "BungeeCord", out.toByteArray());
				} else player.sendMessage((ExcellentBuilding.getInstance().getPrefix() + ExcellentBuilding.getInstance().getConfig().getString("message.server-invalid", "&eThe target server is not running.")).replace("&", "§"));
			}
		}
	}
